...
use Fortblog\Http\Resources\PostsResource;
use Fortblog\FortblogPost;
...
class PostsController
{
    ...
    {
        $entries = FortblogPost::when(request()->has('search'), function ($q) {
    ...
    public function show($id = null)
    {
        ...
        $entry = FortblogPost::with('tags')->findOrFail($id);
    ...
    public function store($id)
    {
        ...
        validator($data, [
            ...
            'author_id' => 'required',
            'title' => 'required',
            'slug' => 'required|'.Rule::unique(config('fortblog.database_connection').'.fortblog_posts', 'slug')->ignore(request('id')),
        ])->validate();

        $entry = $id !== 'new' ? FortblogPost::findOrFail($id) : new FortblogPost(['id' => request('id')]);
    ...
    {
        $entry = FortblogPost::findOrFail($id);
